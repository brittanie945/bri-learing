...
import type { SeedStatus } from "@/lib/api/seeds";
...
function Soil({ size }: { size: number }) {
  ...
  return (
    <ellipse
      ...
      rx={s * 0.36}
      ry={s * 0.08}
      fill="oklch(0.32 0.060 50)"
    />
  );
}

// Unplanted — pulsing soil mound + plus
function Unplanted({ size }: { size: number }) {
  const s = size;
  return (
    <motion.g>
      <Soil size={s} />
...
function SeedDay1({ size }: { size: number }) {
  const s = size;
  return (
    <motion.g
...
function SeedDay2({ size }: { size: number }) {
  const s = size;
  return (
    <motion.g
...
function SeedDay3({ size }: { size: number }) {
  const s = size;
  return (
    <motion.g
...
function Sprouted({ size }: { size: number }) {
  const s = size;
  const petalCount = 6;
...
function Withered({ size }: { size: number }) {
  const s = size;
  return (
    <motion.g
...
// Main export
// ──────────────────────────────────────────
export function SeedAnimation({ status, streakDays, isWatering = false, size = 120 }: Props) {
  return (
